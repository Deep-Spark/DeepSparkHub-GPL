...
if [[ "${TARGET_DIR}" != ""  ]]; then
  ...
  ${PYTHON_PATH} -m pip install --upgrade -t ${PYTHON_DIST_PATH} ${PKG_DIR}/${latest_pkg} --no-deps || exit
  cp -n ./tmp/bin/* ${PYTHON_DIST_PATH}/bin
  rm -rf ./tmp
  echo "Selective_scan installed in ${PYTHON_DIST_PATH}; please add it to your PYTHONPATH."
else
  ${PYTHON_PATH} -m pip uninstall ${PKG_NAME} -y
  ${PYTHON_PATH} -m pip install ${PKG_DIR}/${latest_pkg} --no-deps || exit
fi
