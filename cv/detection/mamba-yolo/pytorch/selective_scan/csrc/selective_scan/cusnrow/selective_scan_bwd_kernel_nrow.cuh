...
void selective_scan_bwd_kernel(SSMParamsBwd params) {
    ...
    for (int chunk = params.n_chunks - 1; chunk >= 0; --chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            for (int r = 0; r < kNRows; ++r) {
                ...
                scan_t running_prefix = chunk > 0 && threadIdx.x % 64 == 0 ? x[(r * params.n_chunks + chunk - 1) * params.dstate + state_idx] : make_float2(1.f, 0.f);
                SSMScanPrefixCallbackOp<weight_t> prefix_op(running_prefix);
                typename Ktraits::BlockScanT(smem_scan).InclusiveScan(
                    thread_data, thread_data, SSMScanOp<weight_t>(), prefix_op
                );
                scan_t running_postfix = chunk < params.n_chunks - 1 && threadIdx.x % 64 == 0 ? smem_running_postfix[state_idx + r * Ktraits::MaxDState] : make_float2(1.f, 0.f);
    ...
    for (int r = 0; r < kNRows; ++r) {
        if (params.dD_ptr != nullptr) {
            __syncthreads();
            dD_val[r] = typename Ktraits::BlockReduceFloatT(smem_reduce_float).Sum(dD_val[r]);
            if (threadIdx.x == 0) { gpuAtomicAdd(&(dD[r]), dD_val[r]); }
        }
        if (params.ddelta_bias_ptr != nullptr) {
            __syncthreads();
            ddelta_bias_val[r] = typename Ktraits::BlockReduceFloatT(smem_reduce_float).Sum(ddelta_bias_val[r]);
...
void selective_scan_bwd_cuda(SSMParamsBwd &params, cudaStream_t stream) {
    if (params.seqlen <= 256) {
        selective_scan_bwd_launch<64, 4, knrows, input_t, weight_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_bwd_launch<64, 8, knrows, input_t, weight_t>(params, stream);
