...
void selective_scan_fwd_kernel(SSMParamsBase params) {
    ...
    for (int chunk = 0; chunk < params.n_chunks; ++chunk) {
        ...
        for (int state_idx = 0; state_idx < params.dstate; ++state_idx) {
            ...
            running_prefix = chunk > 0 && threadIdx.x % 64 == 0 ? smem_running_prefix[state_idx] : make_float2(1.f, 0.f);
...
void selective_scan_fwd_cuda(SSMParamsBase &params, cudaStream_t stream) {
    if (params.seqlen <= 256) {
        selective_scan_fwd_launch<64, 4, input_t, weight_t, output_t>(params, stream);
    } else if (params.seqlen <= 512) {
        selective_scan_fwd_launch<64, 8, input_t, weight_t, output_t>(params, stream);
