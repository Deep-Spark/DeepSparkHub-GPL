...
inline __device__ void store_output1(typename Ktraits::output_t *out,
    ...
    if constexpr (Ktraits::kIsEvenLen) {
        ...
        using vec_t = typename Ktraits::vec_t;
        typename Ktraits::BlockStoreOutputVecT(smem_store_vec).Store(
            reinterpret_cast<vec_t*>(out),
            reinterpret_cast<vec_t(&)[Ktraits::kNLoadsOutput]>(write_vals)
       );
    } else {
        typename Ktraits::BlockStoreOutputT(smem_store).Store(out, write_vals, seqlen);
    }
}
...
inline __device__ void load_output(typename Ktraits::output_t *u,
    ...
    if constexpr (Ktraits::kIsEvenLen) {
        ...
        using vec_t = typename Ktraits::vec_t;
        typename Ktraits::BlockLoadOutputVecT(smem_load_vec).Load(
            reinterpret_cast<vec_t*>(u),
            reinterpret_cast<vec_t(&)[Ktraits::kNLoadsOutput]>(u_vals)
       );
    } else {
        typename Ktraits::BlockLoadOutputT(smem_load).Load(u, u_vals, seqlen, 0.f);
    }
}
